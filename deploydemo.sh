#!/bin/bash
#
# deploydemo.sh - 前端演示环境一键部署脚本（优化版）
set -euo pipefail
...
restore_ip() {
  if [ ${#backup_files[@]} -eq 0 ]; then return 0; fi
  echo ""
  echo ">>> 还原源码中的 IP 配置..."
...
echo "[1/4] 替换源码中的旧 IP ($OLD_IP -> $DEMO_HOST)..."
...
echo ""
echo "[2/4] 准备本地前端产物..."
rm -rf "$LOCAL_BUILD_DIR" "$DEMO_PKG_DIR"
...
echo ""
echo "[3/4] 复制产物到打包目录..."
rsync -a --delete "$LOCAL_BUILD_DIR/" "$DEMO_PKG_DIR/"

echo ""
echo "[4/4] 上传并部署到演示服务器 $DEMO_HOST..."
